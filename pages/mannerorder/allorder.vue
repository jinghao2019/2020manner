<template>
	<view style="height: 100%;background-color: #F5F5F5;">
		<view class="u-f u-f-jsb head px-4" style="background-color: #ffffff;">
		...
		<view>
			<view class="allorder" v-for="(item,index1) in allorder" :key="index1" >
				...
				<view class="order px-4" style="background-color: #ffffff;">	
				<block v-for="(items,index2) in item.data" :key="index2">	
					<view @click="toOrderDetail(index1,index2)">
						<view class="u-f u-f-jsb">
							<view class="u-f" style="width: 400upx;">
								<view style="flex: 2;">{{items.orderstore.name}}自提</view>
								<view style="flex: 1; margin: 0 20upx;">周{{item.datatime|dateFormat}}</view>
							</view>
